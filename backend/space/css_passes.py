...
# locations' pass jobs ask for the CSS TLE at once - it is the same satellite-wide
...
_CSS_TLE_FETCH_LOCK = threading.Lock()
...
class _CelestrakHTTPError(RuntimeError):
    """Raised when Celestrak answers a CSS TLE request with a non-200 status.
...
def clear_css_celestrak_block_flag() -> Dict[str, Any]:
    ...
    logger.info(
        "Celestrak block for CSS manually cleared by operator (was blocked=%s, status_code=%s, reason=%s)",
        ...
        with _CSS_TLE_FETCH_LOCK:
            ...
            for tle_url in CSS_TLE_URLS:
                if celestrak_status.get("blocked") and _is_celestrak_url(tle_url):
                    logger.warning(
                        "Celestrak is flagged as blocked for CSS; skipping Celestrak query until manually reset"
                    )
                    continue
                ...
                try:
                    ...
                    if _is_celestrak_url(tle_url):
                        if isinstance(exc, _CelestrakHTTPError):
                            logger.error(
                                "Celestrak rejected a CSS TLE request with a non-200 response (HTTP %s). "
