...
When UpTonight adds a new target catalogue (e.g., `newCatalogue.yaml`), add the YAML file to `targets/` and run the analysis script:
...
3. The script automatically:
   - Updates `catalogues.conf` from all YAML files present in `targets/`
   - Preserves comments already present in `catalogues.conf`
   - Regenerates `backend/catalogue_aliases.json` for cross-catalogue object matching
4. Save and commit updated files

> [!IMPORTANT]
> **That's it?** No manual edit of `catalogues.conf` is needed anymore, but UpTonight version may still need to be updated.
> 